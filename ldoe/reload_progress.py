...
def spin_result_is_Jack():
    ...
    while (True):
        ...
        if util.CheckFoundPixel(img, 26, 178, 225, 3, quantity = 500):
            print("\tНе джокер", end =" ")
            return False

        img = util.get_window_screen(571, 115, 375, 103)

        try:
            res = cv2.matchTemplate(img, FLIP_THE_CAPS, cv2.TM_CCOEFF_NORMED)
            tem_mat = True
        except:
            tem_mat = False
...
def seach_winning_bet(bet_list):
    ...
    for round_num in range(MAX_ROUNDS):
        if (not isJack):
            ...
            print('\t' + str(round_num) + '. ' + start_cap_balance[:-1] + " -> " + end_cap_balance[:-1] + ": " + result_str[:-1])
    return
...
FLIP_THE_CAPS = cv2.imread('../pic_ldoe/caps/flip_the_caps_res.png', 0)
